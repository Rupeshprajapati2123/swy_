...
const TimesheetForm = () => {
  ...
  const handleExport = () => {
    const projectsData = [];
    const totalWorkForWeek = {};
  
    days.forEach(day => {
      totalWorkForWeek[day] = {
        "Scheduled time": 0,
        "Done on day for all projects": 0
      };
    });
    ...
    for (let i = 0; i < rows; i++) {
      ...
      const projectName = document.getElementById(`project${i + 1}`).value; // Get selected project name
  
      projectData["Project Name"] = projectName;
  
      days.forEach(day => {
        const scheduledTime = 8; // Set scheduled time to 8 hours
  
        const workDone = totalWorkingHours[day][i];
        ...
        totalWorkForWeek[day]["Scheduled time"] += scheduledTime;
        totalWorkForWeek[day]["Done on day for all projects"] += workDone;
      });
  
      projectData["Schedule"] = projectSchedule;
      projectsData.push(projectData);
    }
  
    const exportData = {
      "Projects": projectsData,
      "Total work for week": totalWorkForWeek
    };
    console.log(exportData)
    // Sending a POST request to the backend
    fetch('http://127.0.0.1:8000/api/save_timesheet', {
      method: 'POST',
      headers: {
                      ...
                      <tbody>
                        {[...Array(rows)].map((_, rowIndex) => (
                          <tr key={rowIndex}>
                            ...
                            <td colSpan="2">
                              <input type="text" id={`dept${rowIndex + 1}`}  />
                            </td>
                            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, dayIndex) => (
                              <td key={dayIndex}>
                                <select
